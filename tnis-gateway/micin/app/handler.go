...
// Check authentication token and authorization service
...
// Call microservices API
func CallAPI(w http.ResponseWriter, r *http.Request, service string, method string, version string) {
	e := Execution{name: "CallAPI " + service}
	e.Start()
	...
	resp, _ := resty.R().
		...
		// SetFormData(r).
		SetResult(&Respon{}).
		Post(config.API.Auth + version + "/" + service)
