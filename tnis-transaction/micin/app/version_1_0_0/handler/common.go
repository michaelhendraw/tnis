...
// Elastic Search
func GetTokenDetail(es *elastic.Client, token_param string) model.TokenES {
...
// Call another API
func NotifSendEmail(authorization string, id string) {
	config := config.GetConfig()
	fmt.Println("NotifSendEmail : ", id)
	_, _ = resty.R().
		SetHeaders(map[string]string{
			"authorization": authorization,
		}).
		Get(config.API.Notif + "v100/notif/send_email/" + id)
}
